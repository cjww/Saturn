...
void sa::Swapchain::createSyncronisationObjects() {
...
void sa::Swapchain::create(VulkanCore* pCore, GLFWwindow* pWindow) {
	...
	vk::Format format;
	m_swapchain = pCore->createSwapchain(m_surface, pCore->getGraphicsQueueFamily(), &format);
...
void sa::Swapchain::setResizeCallback(std::function<void(Extent)> function) {
	m_resizeCallback = function;
}

int sa::Swapchain::beginFrame() {
...
void sa::Swapchain::endFrame(vk::Queue queue) {
...
std::vector<vk::ImageView> sa::Swapchain::getImageViews() const {
	return m_imageViews;
}

Extent sa::Swapchain::getExtent() const {
	return m_extent;
}
